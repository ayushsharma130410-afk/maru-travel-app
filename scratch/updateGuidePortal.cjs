...
content = content.replace(
  "import { Award, Phone, Calendar, MapPin, Navigation, User, Car, Info, ShieldAlert, Printer, RefreshCw, Send, CheckSquare, Square, Volume2, AlertCircle } from 'lucide-react';",
  "import { Award, Phone, Calendar, MapPin, Navigation, User, Car, Info, ShieldAlert, Printer, RefreshCw, Send, CheckSquare, Square, Volume2, AlertCircle, Sun, Cloud, CloudRain, CloudSun, CloudSnow, Users, TrendingUp, AlertTriangle } from 'lucide-react';"
);
...
// Add useEffect for reactive guide profile
content = content.replace(
  "// Subscribe to tour changes reactively",
  `// Guide Profile Update Watcher
  useEffect(() => {
...
// Add Progress bar UI and Guest Details UI
content = content.replace(
...
// Weather widget and Guest Details Card
content = content.replace(
  '{/* Date Selector & active schedule */}',
  `{/* Guest Overview Hero Card with Details */}
...
// Remove the old hero card
content = content.replace(
  /<div className="client-hero-card"[\s\S]*?<\/div>\s*\{\/\* Guest Overview Hero Card with Details \*\/\}/,
  "{/* Guest Overview Hero Card with Details */}"
);
...
console.log('Updated GuidePortal.jsx successfully');
